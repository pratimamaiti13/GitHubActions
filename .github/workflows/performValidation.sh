...
perform_validation() {
    ...
    if [ ${TotalLineInDestructiveChanges} > 4 ]; then
        if [ ${TestLevel} = "NoTestRun" ]; then
            sf project deploy start --manifest Delta/package/package.xml --pre-destructive-changes Delta/destructiveChanges/destructiveChanges.xml --target-org $SF_USERNAME --wait 20 --dry-run --json
        elif [ ${TestLevel} = "RunLocalTests" ] || [ ${TestLevel} = "RunAllTestsInOrg" ]; then
            sf project deploy start --manifest Delta/package/package.xml --pre-destructive-changes Delta/destructiveChanges/destructiveChanges.xml --target-org pratimamaiti@nagarro.com --test-level $TestLevel --wait 20 --dry-run --json
        elif [ ${TestLevel} = "RunSpecifiedTests" ]; then
            sf project deploy start --manifest Delta/package/package.xml --pre-destructive-changes Delta/destructiveChanges/destructiveChanges.xml --target-org pratimamaiti@nagarro.com --test-level $TestLevel --tests $TestClasses --wait 20 --dry-run --json
        else
